...
export default [
  ...
  // ── Fork DDD modules: CommonJS, not ESM ──────────────────────────
  //
  // Unlike the upstream monolith, this fork's mcp_server/ is split into
  // CommonJS modules (`module.exports = function register(ctx)` etc.) that
  // are require()d by the loader and the test suite. They still run in the
  // chrome/XPCOM context, so they keep the XPCOM globals but need Node's
  // module/require/exports too. This block overrides the ESM assumption of
  // the extension/** block above for these files.
  {
    files: ["extension/mcp_server/**/*.js"],
    languageOptions: {
      sourceType: "commonjs",
      globals: {
        ...globals.node,
        // ── XPCOM / chrome (same surface as the extension/** block) ──
        Components: "readonly",
        ChromeUtils: "readonly",
        Services: "readonly",
        Cc: "readonly", Ci: "readonly", Cr: "readonly", Cu: "readonly",
